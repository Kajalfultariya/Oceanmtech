...
import { getcframeData, getPaymentData, postAddOrder, postCallOrder } from "../API";
...
const Cframe = () => {
    ...
    const [razorSc, setRazorSc] = useState("")
    ...
    const fetchGetData = async () => {
        const response = await getcframeData();
        ...
        if (response1) {
            if (response1?.status === 200) {
                ...
                setRazorSc(response1.data[0]?.credentials.SANDBOX_SECRET_KEY
                )
            }
    ...
    const makePayment = async () => {
        ...
        formData.append("type", "custom-frame");
                    ...
                    page ?
                        ...
                        <>
                            <div className="digital-div1">
                                ...
                                {frameData?.length === 1 ? "" :
                                    <div>
                                        <img src={frameData[index].image} className="digital-img12center"
                                            id='download-content' />
                                        <div style={{
                                            color: "white",
                                            backgroundColor: "#084277",
                                ...
                                {frameData?.length - 1 === index ?
                                    ...
                                    frameData?.length === 1 ? " " :
                                        <div>
                                            <img src={frameData[index + 1].image} className="digital-img12right" />
                                            <div style={{
                                                color: "white",
                                                backgroundColor: "#084277",
                                                height: "25px",
                        ...
                        dbvcData?.length > 0 ?
                            <div style={{ width: "100%" }}>
                                <div className="input-container">
                                    ...
                                    <input className="input-field" type="text" placeholder="Search Frame" name="search"
                                        value={sName}
                                            ...
                                            <>
                                                <div className="photo-gallery">
                                                    <div style={{
                                                        ...
                                                        <p>{dbvcData[index]?.cards?.length > 1 > 0 ? "More" : " "}   </p>
                                                    </div>
   ...
   /* const result = await axios.post("http://127.0.0.1:8000/razorpay_order", {
         ...
         try {
             const response = await axios.post(url, JSON.stringify({
                 ...
                 "cache-control": "no-cache",
                 "Authorization": "Basic cnpwX3Rlc3RfRXFvMllNNFFOMUhmeWU6UHNScUR0UjRmSnkwUUJBZUxTRlRubjdB"
             })
         ...
         try {
             const response = await axios.post("https://api.razorpay.com/v1/orders", JSON.stringify({
                 amount: 50000,
                 currency: "INR",
                 ...
                 "authorization": 'Basic cnpwX2xpdmVfMGRUWElrN2ZsbmozbEY6SlJWT1FiVFJRT09uc1NwV1Vlck9CTTlC'
             })
         ...
         /*var razorpay = new Razorpay({ key_id: 'rzp_live_0dTXIk7flnj3lF', key_secret: 'JRVOQbTRQOOnsSpWUerOBM9B' })
         ...
         /*    instance.orders.create({
                 ...
                 if (error)
                     console.log("error fgdfgdfg", error)
                 if (response)
        ...
        /* app.use(cors());
         ...
         var options = {
             ...
             "handler": function (response) {
                 console.log("dfsdfsdgfsdg", response);
                 //   var values = {
         ...
         if (response) {
             options.order_id = response.data.order_no;
             //  options.amount = response.data.amount;
             console.log("options", options)
             var rzp1 = new window.Razorpay(options);
             rzp1.open();
         }
         /*  axios.post('https://dev.oceanmtechdmt.in/api/web/v1/order/save',{amount:amount})
               .then(res=>{
